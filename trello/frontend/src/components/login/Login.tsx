...
function Login() {
    ...
    const onchageAccount = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const onContinute = () => {
        if (account?.username == "trello") {
            if (account.password == "trello") {
                setCurrentUser({
                    "id": 0,
                    "username": account?.username,
    ...
    return (
        <div className="w-full h-screen bg-[rgb(250, 251, 252)] my-[50px]">
            <div className="w-[408px] mx-auto  bg-[rgb(255, 255, 255)] shadow-2xl rounded-lg flex flex-col items-center px-8 py-10">
                <div className="h-20 flex items-center flex-col mb-4">
                    <svg
                        viewBox="0 0 94 32"
                        height="32"
                        fill="none"
                    ...
                    <span className="pt-6 font-semibold">Log in to continute</span>
                </div>
                <div className="w-full bg-[#fff7d6] p-2 mb-4" style={{ display: `${!error ? 'none' : "block"}` }}>
                    <span>Incorrect email address and / or password. If you recently migrated your Trello account to an Atlassian account, you will need to use your Atlassian account password. Alternatively, you can get help logging in.Incorrect email address and / or password. If you recently migrated your Trello account to an Atlassian account, you will need to use your Atlassian account password. Alternatively, you can get help logging in.</span>
                </div>
                <div className="w-full ">
                    {/* <form> */}
                    <div className="w-full h-10">
                        <input
                            className="w-full h-full rounded-sm border-2 border-gray-200 p-2 focus:border-teal-500 focus:outline-none"
                            onChange={onchageAccount}
                            type="text"
                            name="username"
                            id="username"
                    ...
                    <div className="w-full h-10 mt-2">
                        <button
                            onClick={onContinute}
                            className="w-full h-full bg-[#0052CC] rounded-sm text-white"
                        >
                            Continute
                        </button>
                    </div>
                ...
                <div className="w-full flex justify-center mt-[24px] mb-[16px]">
                    <h1>Or continute with</h1>
                </div>
                <div className="w-full">
                    <div className="w-full h-10 mb-[16px]">
                        <button className="w-full h-full border-2 border-gray-300 rounded-sm  flex  justify-center items-center">
                            <span className="h-[24px] ">
                                <img
                                    className="w-[30px] h-[24px] pr-2"
                                    src="https://id-frontend.prod-east.frontend.public.atl-paas.net/assets/google-logo.5867462c.svg"
                                    alt=""
                                />
                            </span>
                            <span className=" h-[24px] text-black font-semibold text-center">
                                Google
                            </span>
                        </button>
                    </div>
